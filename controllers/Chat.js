...
exports.SendRequest = (req, res) => {
  Chat.insertMany(
    [
      { ...req.body.first, SenderId: req.userData._id },
      { ...req.body.second, SenderId: req.userData._id },
    ],
    { ordered: true }
  )
    .then((result) => {
      req.app
        .get("socketService")
        .sendTo(req.body.first.ReceiverId, req.body.first.ReceiverId, {
          type: "new-message",
          data: {
            ...req.body.first,
            SenderId: req.userData._id,
...
exports.GetChatUsersRecent = (req, res) => {
  ...
  Chat.aggregate([
    ...
    { $match: { _id: { $ne: ObjectID(req.userData._id) } } },
    // Second Stage
    {
      $lookup: {
        from: "users",
    ...
    },
    {
      $lookup: {
        ...
        let: { client: "$_id", instructor: ObjectID(req.userData._id) },
        pipeline: [
          {
            $match: {
              $expr: {
                $and: [
                  { $eq: ["$Program.createdBy", "$$instructor"] },
                  { $eq: ["$UserId", "$$client"] },
                ],
              },
            },
